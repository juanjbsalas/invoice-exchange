...
export default function AdminUsersPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-surface-0">User Management</h1>
      ...
      {/* Tabs */}
      <div className="flex gap-1.5 border-b border-surface-600 pb-3">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-surface-100">
                {FARMERS.filter((f) =>
                  ...
                  return (
                    <tr key={f.id} className={`hover:bg-surface-800 transition-colors ${isSuspended ? "opacity-60" : ""}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2.5">
                          <div className="h-8 w-8 rounded-full bg-emerald-100 flex items-center justify-center shrink-0">
                            <Sprout className="h-4 w-4 text-emerald-600" />
                          </div>
                          <span className="font-medium text-surface-0">{f.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-surface-600">{f.location}</td>
                      <td className="px-6 py-4 text-surface-200">{f.invoiceCount}</td>
                      <td className="px-6 py-4 font-semibold text-surface-0">{formatCurrency(f.totalInvoiced)}</td>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-surface-100">
                {GROCERY_STORES.filter((s) =>
                  ...
                  return (
                    <tr key={s.id} className={`hover:bg-surface-800 transition-colors ${isSuspended ? "opacity-60" : ""}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="font-medium text-surface-0">{s.name}</p>
                      ...
                      <td className="px-6 py-4 font-semibold text-surface-0">{formatCurrency(s.totalOwed)}</td>
                      <td className="px-6 py-4">
                        <Badge variant={isSuspended ? "danger" : "success"}>
                          {isSuspended ? "Suspended" : "Active"}
                        </Badge>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-surface-100">
                {MOCK_INVESTORS.filter((i) =>
                  ...
                  return (
                    <tr key={inv.id} className={`hover:bg-surface-800 transition-colors ${isSuspended ? "opacity-60" : ""}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="font-medium text-surface-0">{inv.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 font-semibold text-surface-0">{formatCurrency(inv.balance)}</td>
                      <td className="px-6 py-4 font-semibold text-surface-0">{formatCurrency(inv.totalInvested)}</td>
                      <td className="px-6 py-4 text-surface-200">{inv.active}</td>
                      <td className="px-6 py-4">
                        {isSuspended ? (
